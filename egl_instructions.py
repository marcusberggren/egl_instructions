...
def open_si(path=None):
    root = tk.Tk()
    root.lift()
    root.withdraw()
    file = filedialog.askopenfile(initialdir=path, title="select SI-file", filetypes=[("Excel files", ".xlsm")])
    root.quit()
...
if __name__ == '__main__':
    vgm_file =r"docs\si\RDX1 005S SEGOT.xlsm"
    si_file = r"docs\si\RDX1 005S SEGOT.xlsm"
    vgm_instructions(vgm_file)
    #shipping_instructions(si_file)
